...
#include "PWGameSession.generated.h"

const FName PWGameSessionName(TEXT("Game"));
const FName PWPartySessionName(TEXT("Party"));

const FName PWGamePort(TEXT("GamePort"));
const FName PWBeaconPort(TEXT("BeaconPort"));

struct FPWGameSessionParams
{
...
UCLASS(config = Game)
class PWGAME_API APWGameSession : public AGameSession
{
	GENERATED_BODY()
...
protected:
	...
	FPWGameSessionParams CurrentSessionParams;
	/** Current host settings */
	TSharedPtr<class FPWOnlineSessionSettings> HostSettings;
	/** Current search settings */
	TSharedPtr<class FPWOnlineSearchSettings> SearchSettings;
	...
	*/
	DECLARE_EVENT_TwoParams(AP906_GameSession, FOnCreatePresenceSessionComplete, FName /*SessionName*/, bool /*bWasSuccessful*/);
	...
	*/
	...
	DECLARE_EVENT_OneParam(AP906_GameSession, FOnJoinSessionComplete, EOnJoinSessionCompleteResult::Type /*Result*/);
	FOnJoinSessionComplete JoinSessionCompleteEvent;

	/*
	* Event triggered after session search completes
	*/
	//DECLARE_EVENT(AP906_GameSession, FOnFindSessionsComplete);
	DECLARE_EVENT_OneParam(AP906_GameSession, FOnFindSessionsComplete, bool /*bWasSuccessful*/);
...
public:
	...
	*/
	bool HostSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers);
